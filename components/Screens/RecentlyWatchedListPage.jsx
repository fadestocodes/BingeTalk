...
const RecentlyWatchedScreen = () => {
    ...
    const { data : recentlyWatched, loading, refetch, fetchMore, hasMore, removeItem  } = useRecentlyWatched(userId)
    const { data : currentlyWatchingItems, loading:loadingCurrentlyWatching, refetch : refetchingCurrentlyWatching, fetchMore:fetchMoreCurrentlyWatching, hasMore:hasMoreCurrentlyWatching, removeItem : removeCurrentlyWatchingItem } = useGetCurrentlyWatchingItems(userId)
  ...
  if (loading && loadingCurrentlyWatching ){
    return(
     ...
     <TouchableOpacity onPress={()=>router.back()} style={{justifyContent:'flex-start', alignSelf:'flex-start' }}>
         <BackIcon size={22} color={Colors.mainGray} />
            ...
            <View className="flex-row w-full justify-start items-center gap-2 py-1">
                <Clock9 color='white'  />
                <Text className='text-white text-3xl  font-pbold'>Recently Watched</Text>
            </View>
            <View className='flex-row gap-2 w-full justify-start items-center'>
            <TouchableOpacity onPress={()=>{setTab('watched') }} style={{ borderRadius:15, backgroundColor:tab==='watched' ? 'white' : 'transparent', paddingHorizontal:8, paddingVertical:3, borderWidth:1, borderColor:'white' }}>
                    <Text className=' font-pmedium' style={{ color : tab==='watched' ? Colors.primary : 'white' }}>Watched</Text>
                </TouchableOpacity>
            <TouchableOpacity onPress={()=>{setTab('currentlyWatching') }} style={{ borderRadius:15, backgroundColor:tab==='currentlyWatching' ? 'white' : 'transparent', paddingHorizontal:8, paddingVertical:3, borderWidth:1, borderColor:'white' }}>
                    <Text className=' font-pmedium' style={{ color : tab==='currentlyWatching' ? Colors.primary : 'white' }}>Currently Watching</Text>
                </TouchableOpacity>
                </View>
            <View >
            { tab === 'watched' ? (
<>
                ...
                { currentlyWatchingItems.length < 1 ? (
                    ...
                    refreshControl={
                        <RefreshControl
                            tintColor={Colors.secondary}
                            refreshing={loadingCurrentlyWatching}
                            onRefresh={refetchingCurrentlyWatching}
                        />
                    }
                        scrollEnabled={true}
