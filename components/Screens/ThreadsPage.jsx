...
import { ArrowDownIcon, ArrowUpIcon, MessageIcon } from '../../assets/icons/icons'
...
const ThreadsIdPage = () => {
  ...
  return (
    <SafeAreaView className='h-full pb-32 ' style={{backgroundColor:Colors.primary}} >
        ...
        <View style={{gap:10, marginVertical:10, paddingTop:0, paddingHorizontal:20}}  >
          <View className='gap-3'>
            <Text className="text-white  font-pbold text-2xl leading-6  ">{mainThread.title}</Text>
            { mainThread.caption && (
              <View className='gap-3'>
                <Text className='text-secondary text-lg leading-5 font-pcourier uppercase text-center'>{mainThread.user}</Text>
                <Text className="text-white text-lg leading-5 font-pcourier">{mainThread.caption}</Text>
              </View>
            ) }
            ...
            renderItem={({item}) => {
                ...
                <View className={`pt-5 mt-5 mb-3  gap-3 px-3 }`} style={{borderTopWidth:.2, borderColor:Colors.mainGray}}>     
                    ...
                    <View className="mb-0 gap-3">
                        ...
                        <Text className='text-white text-lg leading-5 font-pcourier'>{item.comment}</Text>
                        <View className='flex-row gap-3 '>
                            <TouchableOpacity className='flex-row gap-3 ' style={{borderRadius:10, paddingHorizontal:10, paddingVertical:5, borderWidth:1, borderColor:Colors.mainGray, justifyContent:'center', alignItems:'center'}}>
                        ...
                        <FlatList
                            ...
                            renderItem={({item : replyItem}) => (
                                ...
                                <View className='ml-4 pl-5 pt-4 pb-5 border-l-[.2px] border-secondary gap-3'>
                                    <Text className='text-secondary text-lg leading-5 font-pcourier uppercase text-center'>{replyItem.user}</Text>
                                    <Text className='text-white text-lg leading-5 font-pcourier'>{replyItem.comment}</Text>
                                    <View className='flex-row gap-3'>
                                        <TouchableOpacity className='flex-row gap-3 ' style={{borderRadius:10, paddingHorizontal:10, paddingVertical:5, borderWidth:1, borderColor:Colors.mainGray, justifyContent:'center', alignItems:'center'}}>
                            ...
                            ListFooterComponent={(
                                ...
                                <TouchableOpacity className='rounded-md bg-mainGray  py-1 w-full ' onPress={()=>loadMoreReplies(item.id)}>
