    ...
    <SafeAreaView className='bg-primary w-full h-full'>
        <ScrollView showsVerticalScrollIndicator={false} >
            <View className='items-center justify-center w-full'>
              <Text className='text-white'>UserBadgesPage</Text>
              <FlatList
                data={fakeBadges}
                scrollEnabled={false}
                ...
                renderItem={({item}) => {
                  ...
                  return (
                  <TouchableOpacity className={`bg-primaryLight w-[160px] h-[230px] px-4 rounded-2xl justify-center items-center gap-3 relative ${item.currentLevel === 'NONE' ? 'opacity-30' : ''} `}>
